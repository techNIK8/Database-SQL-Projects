...
CONSTRAINT afm FOREIGN KEY (AFM) REFERENCES company(AFM) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;
...
CREATE TABLE has(
titlos_degree VARCHAR(50) NOT NULL,
idryma_degree VARCHAR(40) NOT NULL,
candid varchar(12) NOT NULL,
Date_of_apoktisi YEAR NOT NULL,
Grade INT NOT NULL,
PRIMARY KEY(titlos_degree,idryma_degree,candid),
CONSTRAINT titlos_idryma FOREIGN KEY (titlos_degree,idryma_degree)
REFERENCES degree(titlos,idryma)
ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT candid FOREIGN KEY (candid)
...
username_of_recruiter VARCHAR(12) NOT NULL,
start_date DATE NOT NULL,
salary FLOAT(6,1),
title VARCHAR(40),
workplace VARCHAR(45),
Date_of_publication DATETIME DEFAULT NOW(),
Date_of_expiration DATE NOT NULL,
PRIMARY KEY (id),
CONSTRAINT JOB_RECR FOREIGN KEY (username_of_recruiter) REFERENCES recruiter(username) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;
...
job_id int(4) NOT NULL,
candidate_interviewed VARCHAR(12) NOT NULL,
recruiter_responsible VARCHAR(12) NOT NULL,
date DATE NOT NULL,
time TIME NOT NULL,
duration TIME NOT NULL,
comments TINYTEXT,
PRIMARY KEY(recruiter_responsible,candidate_interviewed,job_id),
CONSTRAINT recruitt FOREIGN KEY (recruiter_responsible) REFERENCES recruiter(username)
ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT candidd FOREIGN KEY (candidate_interviewed)
REFERENCES candidate(username)
ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT job_request FOREIGN KEY (job_id) REFERENCES job(id)
ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;


CREATE TABLE evaluation (
candidate_being_evaluated VARCHAR(12) NOT NULL,
recruiter_in_charge VARCHAR(12) NOT NULL,
personality ENUM('1','2','3','4','5') NOT NULL,
education ENUM('1','2','3','4','5') NOT NULL,
experience ENUM('1','2','3','4','5') NOT NULL,
PRIMARY KEY(recruiter_in_charge,candidate_being_evaluated),
CONSTRAINT recruit2 FOREIGN KEY (recruiter_in_charge)
REFERENCES interview(recruiter_responsible)
ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT candid2 FOREIGN KEY (candidate_being_evaluated)
REFERENCES interview(candidate_interviewed)
ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;
...
section_description TINYTEXT NOT NULL,
parent_section INT,
PRIMARY KEY(section_id,etairia_AFM),
CONSTRAINT parent_id FOREIGN KEY (parent_section)
REFERENCES sections(section_id)
ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT etairia FOREIGN KEY (etairia_AFM)
...
CREATE TABLE is_active_on(
section_id INT NOT NULL AUTO_INCREMENT,
AFM char(9) NOT NULL,
PRIMARY KEY(section_id),
CONSTRAINT sectionidd FOREIGN KEY (section_id)
REFERENCES sections(section_id)
ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT afmm FOREIGN KEY (AFM)
REFERENCES company(AFM)
ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;
...
insert into company (AFM, DOY, name, tel, street, num, city, country) values
...
insert into recruiter (username, exp_years, AFM) values
('msmith', 4, '18765549'),
...
insert into has (candid, titlos_degree, idryma_degree, Date_of_apoktisi, grade) values
...
insert into job (start_date, salary, title, workplace, username_of_recruiter, Date_of_publication, Date_of_expiration) values
